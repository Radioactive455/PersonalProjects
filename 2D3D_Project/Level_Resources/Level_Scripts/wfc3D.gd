class_name WFC3D extends Resource
...
# BACK = Vector3i(0, 0, 1)
var direction_to_index : Dictionary = {
	  Vector3i.LEFT : 2
	, Vector3i.RIGHT : 3
	, Vector3i.FORWARD : 1
	, Vector3i.BACK : 0
...
var mesh_rotation_convert : Dictionary = {
	0 : 0
	, 1 : 16
	, 2 : 10
	, 3 : 22
...
func init_wfc(new_size : Vector3i, all_prototypes : Dictionary, custom_seed : String):
...
func get_min_entropy_coords():
	...
	for _z in wave_function:
		...
		pos_in_func.z += 1
	# end for loop _z
	return min_positions[randi_range(0, min_positions.size() - 1)]
